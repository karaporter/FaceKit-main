...
def readMesh(mesh, fileName, normalize = True):
    ...
    mesh.n_vertex = len(mesh.vertices)           
    mesh.point_per_face = 0 if len(mesh.faces) == 0 else len(mesh.faces[0])
    mesh._compute_bbox()
    

    if normalize:
        mesh.vertices = normalizeCoord(mesh.vertices, mesh.bbox)
        mesh._compute_bbox()
